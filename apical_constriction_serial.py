...
from vedo import Box, Spheres, Arrows, show, ProgressBar
        ...
        for j in range(n):
            if i != j:
                ...
                if dist < r_max:
                    ...
                    prodi = (pix * rx + piy * ry + piz * rz) / \
                        dist + cos(preferential_angle)
                    ...
                    dBtheta = -prodi*(cos(Xitheta) * sin(r_hat_theta) *
                                      cos(Xiphi - r_hat_phi) - sin(Xitheta) * cos(r_hat_theta))
                    sin_Xi_theta = sin(Xitheta)
                    if abs(sin_Xi_theta) > 1e-10:
                        dBphi = -prodi*(-sin(r_hat_theta) *
                                        sin(Xiphi - r_hat_phi) / sin_Xi_theta)
...
pb = ProgressBar(0, int(T/output_int), title="t=")
for t_interval in pb.range():
...
for t in range(len(coords_t)):
    ...
    polarities = Arrows(start_pts=coords_t[t],
                        end_pts=coords_t[t] + pol_t[t], c='tomato')
    show(world, cells, polarities, interactive=t <= 0, viewup='z')
